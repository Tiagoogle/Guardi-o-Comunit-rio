...
2. **Triage (Classificação Inteligente & SLA)**:
   ...
   - Define o SLA (Prazo de Resposta) recomendado para garantir consistência.
...
export const analisarInteracao = async (inputText: string, canalOrigem: string): Promise<MultiAgentSystemResult> => {
  ...
  try {
          ...
          properties: {
            ...
            triage: {
              ...
              properties: {
                ...
                sla_recommendation: { type: Type.STRING }
              }
            },
            analyst: {
              type: Type.OBJECT,
              properties: {
                ...
                silent_risk_detected: { type: Type.BOOLEAN }
              }
            },
            strategist: {
              type: Type.OBJECT,
              properties: {
                ...
                proactive_mitigation_plan: { type: Type.STRING }
              }
            },
            communicator: {
              type: Type.OBJECT,
    ...
    return JSON.parse(cleanJsonOutput(rawText)) as MultiAgentSystemResult;
